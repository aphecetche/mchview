...
const OutlineSelector = {
  ...
  view: () => {
    return m('ul', {class: 'outlineselector'},
      Object.keys(ShowOutline).map(function(u) {
        ...
        return m('li', [
          m('input[type=checkbox]',
            {id: u, checked: c, onclick: (e) => {
              ShowOutline[e.target.id] = !ShowOutline[e.target.id];
            }}),
          m('label', {for: u}, u),
        ]);
      }),
      m('div', {class: 'outlineselector-buttongroup'},
        m( 'button', {onclick: () => {
          ShowOutline.setAll();
        }, disabled: ShowOutline.all()}, 'All'),
        m( 'button', {onclick: () => {
          ShowOutline.setNone();
        }, disabled: ShowOutline.none()}, 'None')
      )
    );
  }
};
